#!/usr/bin/env bash
# repo-watch.sh — Polls Gitea issues/PRs and invokes Goose to handle them.
#
# Watches for open issues and PRs assigned to this agent on its Gitea fork.
# When a human posts, comments, or leaves a review, invokes Goose with
# the full conversation context. Goose decides what to do: respond, code,
# open a PR, merge, etc.
#
# Usage:
...
#
# Prerequisites:
#   - Goose installed and configured (`goose` must work)
#   - Environment variables set by entrypoint: GITEA_URL, GITEA_TOKEN,
#     GITEA_USER, REPO_NAME
#   - Provider API key set (e.g. ANTHROPIC_API_KEY or OPENAI_API_KEY)
#
...
# Build prompt and invoke goose for a given issue/PR.
...
CONTEXT
    ...
    local log_file
    log_file="${LOG_DIR}/${item_type,,}-${number}-$(date '+%Y%m%d-%H%M%S').log"
    LAST_LOG_FILE="$log_file"
    ln -sfn "$log_file" "$CURRENT_LOG_LINK"

    log "invoking goose for ${item_type} #${number}..."
    log "log: ${log_file}"
    ...
    # Build goose args
    # Goose uses: goose run -i <file> --no-session
    local goose_args=(run -i "$task_file" --no-session)
    ...
    if [[ -n "$MATCHED_COMMAND" ]]; then
        ...
        while IFS= read -r flag; do
            [[ -n "$flag" ]] && goose_args+=("$flag")
        done <<< "$agent_flags"
    fi

    [[ -n "$REPO_WATCH_MAX_TURNS" ]] && goose_args+=(--max-turns "$REPO_WATCH_MAX_TURNS")

    # Goose needs GOOSE_MODE=auto for full autonomy
    local goose_env=(
        "GOOSE_MODE=auto"
    ...
    if [[ -n "$REPO_WATCH_TIMEOUT" ]]; then
        (set +e; cd "$REPO_DIR" && env "${goose_env[@]}" timeout --signal=TERM "$REPO_WATCH_TIMEOUT" \
            goose "${goose_args[@]}") > "$log_file" 2>&1 || rc=$?
    else
        (set +e; cd "$REPO_DIR" && env "${goose_env[@]}" goose "${goose_args[@]}") \
            > "$log_file" 2>&1 || rc=$?
    fi

    rm -f "$task_file" "$CURRENT_LOG_LINK"

    # Print tail of output for terminal visibility
    if [[ -f "$log_file" ]]; then
        tail -20 "$log_file" 2>/dev/null || true
    fi

    if [[ "$rc" -eq 124 ]]; then
        log "goose timed out after ${REPO_WATCH_TIMEOUT} for ${item_type} #${number}"
        return 0  # timeout is deliberate, not a retry-worthy failure
    elif [[ "$rc" -ne 0 ]]; then
        log "goose exited with error (code ${rc}) for ${item_type} #${number}"
        # Track failure count
        mkdir -p "$FAIL_DIR"
...
# Truncate a goose log into a readable markdown file for attachment.
# Goose outputs plain text, so we capture it as-is with a header.
...
# Ensure the agent posted a comment on the issue after invocation.
# If goose didn't comment (e.g. weaker model that skips Gitea API calls),
# post a fallback so the who-spoke-last check advances past the agent
# and prevents infinite re-triggering on the same issue.
# Args: $1=issue_number $2=pre_invocation_comment_count
ensure_agent_commented() {
    local number="$1" pre_count="$2"

    local post_count
    post_count=$(gitea GET "/repos/${REPO_PATH}/issues/${number}/comments" \
        | jq "[.[] | select(.user.login == \"${GITEA_USER}\")] | length")

    if [[ "$post_count" -le "$pre_count" ]]; then
        log "agent did not comment on #${number}, posting fallback"
        gitea POST "/repos/${REPO_PATH}/issues/${number}/comments" \
            '{"body":"Agent completed work, but failed to reply on this issue. Please check the activity log or branch."}' >/dev/null
    fi
}
...
if ! command -v goose &>/dev/null; then
    echo "Error: goose is not installed. Run the install script first." >&2
    exit 1
fi
...
while true; do
    ...
    for i in $(seq 0 $((count - 1))); do
        ...
        # Count agent comments before invocation (for fallback detection)
        pre_agent_comments=$(echo "$comments_json" | jq "[.[] | select(.user.login == \"${GITEA_USER}\")] | length")
        ...
        # Ensure agent commented (prevents infinite re-trigger if model skips Gitea API)
        ensure_agent_commented "$number" "$pre_agent_comments"
...
"
            ...
            # Ensure agent commented (prevents infinite re-trigger if model skips Gitea API)
            ensure_agent_commented "$pr_number" "$pre_agent_comments"
